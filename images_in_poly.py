...
def copy_to_destination(image_path, source_directory, destination_directory):
    ...
    os.makedirs(os.path.join(destination_directory, os.path.dirname(relative_path)), exist_ok=True)
...
def arg_parse():
    ...
    """
    parser = argparse.ArgumentParser(description="Script to find in which polygon a geolocalized image belongs to")
    parser.add_argument("-v", "--version", action='version', version='release 1.0')
    parser.add_argument("-j", "--json_file", help="Path to the geojson file", required=True)
    parser.add_argument("-s", "--source", help="Path to the images folder. Sub folder are scanned too", required=True)
    parser.add_argument("-d", "--destination", help="Path to the destination folder in which the images will be copied")
...
def main():
    ...
    for image in images_list:
        ...
        if city is not None and args.destination:
            ...
            copy_to_destination(image[0], args.source, os.path.join(args.destination, city))
